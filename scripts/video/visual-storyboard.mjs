// Reviewed against actual final captures before narration/rendering. No provider calls.
export const visualVideo = {
  title: "DUIT · The card comes first",
  privacy:
    "DUIT · Cards worth keeping",
  scenes: [
    {
      id: "visual-01",
      seconds: 19,
      title: "Your card wallet.",
      captureKey: "wallet",
      frame: "phone",
      narration:
        "DUIT starts with the card. Open your wallet and browse faces, businesses and cards you can recognise. A little less scrolling through job titles; a little more, “Ah, that’s the person I met.”",
    },
    {
      id: "visual-02",
      seconds: 16,
      title: "First, the person.",
      captureKey: "maya-person",
      frame: "phone",
      narration:
        "First, the person. A proper portrait, a name, and a short introduction. The aim is to recognise someone in a second. Their entire career can wait politely outside.",
    },
    {
      id: "visual-03",
      seconds: 19,
      title: "Then, their card.",
      captureKey: "maya-card",
      frame: "phone",
      narration:
        "Next, the visiting card. Keep its colours, its layout, and its character. It is the design they chose to share, shown clearly as an image. The original should stay available, not vanish behind a wall of extracted text.",
    },
    {
      id: "visual-04",
      seconds: 18,
      title: "Show the business.",
      captureKey: "maya-pitch",
      frame: "phone",
      narration:
        "Then, a look inside the business. What do they do? Who do they help? What would you talk about next? A short visual pitch and a clear button give the introduction somewhere useful to go.",
    },
    {
      id: "visual-05",
      seconds: 16,
      title: "Same rhythm. Different character.",
      captureKey: "second-profile",
      frame: "phone",
      narration:
        "Another person should feel like another business. Different face, different card, different visual story. The same simple three-page structure holds it together, without dressing every company in exactly the same suit.",
    },
    {
      id: "visual-06",
      seconds: 19,
      title: "Keep the conversation.",
      captureKey: "meeting-memory",
      frame: "phone",
      narration:
        "The meeting still matters. Saved notes, the place, the date, and a promised next step sit behind the card. You can remember the conversation without turning the first hello into paperwork. Memory supports the introduction; it does not take over.",
    },
    {
      id: "visual-07",
      seconds: 18,
      title: "Open. Meet. Explore.",
      captureKey: "recipient-web",
      frame: "phone",
      narration:
        "And the recipient can open the card in a phone browser. No installation before saying hello. Move from the person to the visiting card to the pitch, then choose the next step. This is the actual local web flow.",
    },
    {
      id: "visual-08",
      seconds: 23,
      title: "A few next ideas.",
      frame: "title",
      points: [
        "An optional hello video",
        "AI help, with your review",
        "Keep the original",
      ],
      captureLabel: "Proposed next ideas · AI is not enabled",
      narration:
        "What could come next? Perhaps a short owner-recorded introduction, played only when the visitor chooses. AI could help tidy images and drafts behind the scenes, with review and the original kept safe. Those are ideas to test. AI is not connected yet.",
    },
  ],
};
export const videos = { visual: visualVideo };
