...
The download page shows the APK version and original GitHub publication time in IST. `/downloads/release.json` reads the current channel manifest containing the public version, publication time, filename and download URL. The release publisher verifies the versioned APK checksum and stamps GitHub's original publication time into this manifest; older manifests use a compatibility lookup against GitHub's API. It caches successful lookups for five minutes. A temporary metadata outage does not disable the APK download. Website redeploys do not change the displayed APK release time.

All APK deliveries use `DUIT-2026-<version>.apk`, including local build output, GitHub release assets and cloud build ZIP contents. The Render download button follows the verified versioned asset. The old `DUIT-2026-Pilot.apk` release alias remains available for existing links; normal downloads use the versioned filename.
