...
| Video capture                                     | Six actual browser sequences recorded; clear narration generated; final native clips and rendering in progress                        | Ignored `artifacts/videos/captures`                                         |
...
2. Retest affected native paths after the Mac is unlocked. Finish and inspect the main and quick narrated videos; use accurately labelled actual app-web recordings while native capture is unavailable.
3. Commit/push milestones, run the repository pull-request checks and resolve actual failures before merging.
